...
readonly class CacheClearTool
{
    ...
    public function execute(string $scope = 'pages', int $pageId = 0): CacheClearedResult
    {
        if (!in_array($scope, ['pages', 'all', 'page'], true)) {
            throw new ToolCallException('Invalid scope: ' . $scope . '. Valid scopes: pages, all, page');
        }

        if ($scope === 'page' && $pageId === 0) {
            throw new ToolCallException('pageId is required when scope is "page"');
        }
        ...
        if ($scope === 'all' && !$this->permissionService->isAdmin()) {
            throw new ToolCallException('Clearing all caches requires administrator privileges.');
        }
