...
- `Service/DataHandlerService` — Wraps TYPO3 DataHandler for create/update/delete operations (single and batch). `createRecord`/`updateRecord`/`updateRecords` run the field data through `MmFieldNormalizer` first, so every writer (generated tools, `pages_*`/`content_*`, `record_update_batch`) accepts MM fields in the same shapes.
...
- `Service/TcaSchemaService` — TCA field metadata extraction for schema introspection and dynamic tools. `select`/`group` columns are readable and writable whether or not they have an `MM` table; `getMMFields()` names the ones that do, and the schema marks them with `relation: mm`, `mm`, `foreignTable`/`allowed`, `minitems`/`maxitems`. `inline`, `file`, `category` and select/group **without** MM are unchanged.
