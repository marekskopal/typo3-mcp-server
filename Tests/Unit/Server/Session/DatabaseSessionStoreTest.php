...
final class DatabaseSessionStoreTest extends TestCase
{
    ...
    public function testExistsReturnsFalseForUnknownSession(): void
    {
        ...
        $store = new DatabaseSessionStore($repository, 86400);
    ...
    public function testExistsReturnsTrueAndTouchesSlidingTtl(): void
    {
        ...
        $repository = $this->createMock(McpSessionRepository::class);
        $repository->method('findBySessionId')->with($this->sessionId)->willReturn($row);
    ...
    public function testExistsDeletesAndReturnsFalseWhenExpired(): void
    {
        $row = [
            ...
            'last_activity' => time() - 172_800,
        ];

        $repository = $this->createMock(McpSessionRepository::class);
        $repository->method('findBySessionId')->willReturn($row);
    ...
    public function testWriteUpsertsWithCurrentTimestamp(): void
    {
        $repository = $this->createMock(McpSessionRepository::class);
        $repository->expects(self::once())
            ->method('upsert')
            ->with($this->sessionId, 'data', self::isInt());
    ...
    public function testDestroyDelegatesToRepository(): void
    {
        $repository = $this->createMock(McpSessionRepository::class);
