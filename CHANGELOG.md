...
### Added
- **TypoScript tools.** Nothing in the tool surface reached `sys_template`, and it never could have arrived through auto-discovery either — `ExtensionTableDiscoveryService` excludes the whole `sys_` prefix — so an agent could read and rewrite content but not the configuration that decides how any of it renders. The new `typoscript_*` group adds CRUD on `sys_template` (`typoscript_list` / `_get` / `_create` / `_update` / `_delete`, built on the same `TableToolFactory` as the redirect and scheduler groups, with `constants` and `config` as ordinary writable fields) plus two read-only views that a record listing cannot give: `typoscript_rootline` reports which templates apply to a page and in what order — including the site and its sets, which have provided TypoScript ahead of any record since v13.1 — and `typoscript_active` returns the effective, compiled constants and setup as a flat map of dotted paths, constants already substituted and conditions resolved, the equivalent of the backend's *Active TypoScript* view. `typoscript_active` takes `type` (`setup`, `constants`, `both`) and an optional `path` to return one object path and its children, and caps its answer at 2000 entries per section with `"truncated": true` rather than returning megabytes. Compilation follows the sequence core's own non-frontend caller (`BackendConfigurationManager`) uses, so conditions, workspaces and site sets resolve as the backend would; conditions reading the HTTP request take their default branch, since there is no request. No batch or move tool: a `sys_template` move silently changes which pages inherit it.

### Changed
- **A tool failure is now an MCP tool error, never a successful result describing one.** About half the tool surface returned a hand-built `json_encode` string and encoded refusals as `{"error": "..."}` payloads — `record_search`, `record_count`, `pages_search`, `content_search`, `table_schema`, `file_search`, the generated `*_get`, the workspace tools — while the other half returned typed DTOs. A client sees a `{"error": ...}` body as a *successful* call, so an agent reads it as data and does not retry; the two conventions also drifted apart wherever they were hand-copied. Every tool now returns a typed result object from `Classes/Tool/Result/` and throws `ToolCallException` when it could not do what was asked: a malformed `fields` or `search` payload, an unknown table or field, a denied permission, or a write whose target does not exist. What the tool *did* do is still data, including a read that matched nothing — `pages_get`, `content_get`, `backend_user_get`, `backend_group_get`, `workspace_get`, `permission_check_page` and the generated `*_get` now answer a missing uid with `{"found": false, "table": ..., "uid": ..., "message": ...}` in place of `{"error": ...}`, which is the only payload shape that changed. Every other response is byte-identical to before; the DTOs exist for type safety, not to reshape the wire format. `ErrorResult` is gone.
- **`No valid fields provided` now names the table and the fields that were dropped.** The six writers that filter a `fields` payload against the writable columns each carried their own copy of the refusal; they now share `FieldRejection::noValidFields()`, which reports `No valid fields provided for table "pages". Ignored, not writable: bogus. Call table_schema to see which fields can be written.` Previously the dropped names were in an `ignoredFields` key that a client had to think to read.

### Security
- **`ctrl.adminOnly` is now honoured on the read path.** `PermissionService::canSelectTable()` consulted only the `tables_select` grant, so a non-admin who held it on a table TYPO3 marks `adminOnly` — `sys_template` above all — could read the table through `record_search`, `record_count`, `table_schema` and the schema resources. That grant is not supposed to be enough: core's list module hides such a table from a non-admin entirely (`RecordListController`), and `DataHandler::checkModifyAccessList()` refuses their writes outright, so only the read side disagreed. It now refuses too, which is the same class of fix as the `non_exclude_fields` one below. `permission_check_table` reports `canSelect: false` for such a table and `permission_check_summary` leaves it out of `tablesSelect`, so the tools that exist to predict the read path say what it will do. Administrators are unaffected, and a table without the flag behaves exactly as before.
- **Record content no longer reaches the audit log.** `AuditLogger` dropped array arguments so that "free-text content never lands in the log", but nearly every write tool takes its payload as a JSON *string* (`fields` on `pages_update`, `content_create`, `record_update_batch`, the generated `*_create` / `*_update`; `search` on the search tools), so the string branch kept the first 100 characters of it — a bodytext, an e-mail address, whatever was being written — in `sys_log`, which every administrator can read and which installations routinely ship to an external log collector. A string holding a JSON object is now reduced to its top-level field names (`{header, bodytext, hidden}`), capped at 20 names, with control characters stripped. Scalars, uid lists and plain-text search terms are unchanged, and a malformed payload is still recorded literally so the failure stays diagnosable.
- **`file_upload_from_url` authorizes the destination before it fetches anything.** The storage and folder were resolved *after* `downloadToFile()`, so the checks that refuse an inaccessible storage or a path outside the user's filemounts ran only once up to 100 MB had already been pulled over five redirect hops. A backend user with no file access at all could therefore make the server fetch from any public URL on every tool call and only then be told no. The storage, the folder and the user's `file_permissions` (via `checkFolderActionPermission('add', …)`) are now checked first, so a denial costs no network I/O. The SSRF hardening on the fetch itself is unchanged.
- **The schema resources honour `tables_select`.** The `table_schema` tool has always refused a table the backend user may not read, but the MCP resources exposing the same information did not: `typo3://schema/tables/{tableName}` returned the full field schema of any TCA table, and `typo3://schema/tables` listed every table in the installation. A non-admin could therefore map structures the backend never shows them — `be_groups`, the `sys_*` tables, extension tables outside their grant. Both now apply `PermissionService::canSelectTable()`: the template refuses with "Access denied" before it reveals whether the table exists, and the listing omits what the user cannot read. Administrators pass the check unconditionally, so their view is unchanged. No row data was ever exposed by either resource.
- **`typo3://system/info` no longer hands the install path to every user.** The resource returned `projectPath` (`Environment::getProjectPath()`, the absolute filesystem path of the installation) and `applicationContext` to any authenticated MCP user, including a non-admin editor. Core gates its System Information toolbar — which shows exactly these — on `isAdmin()`, and a non-admin never sees the path in the backend at all; path disclosure is also the classic multiplier for a later file-write or include bug, while nothing in the MCP surface addresses files by filesystem path. Both fields are now `null` for non-administrators. `typo3Version` (in the About module for every backend user), `phpVersion` and `os` are unchanged, so a client can still tell which API shapes apply.
- **The workspace tools check read access on the table they are given.** `workspace_publish`, `workspace_discard` and `workspace_stage_set` take `table` as a free-form parameter and looked the version up with every query restriction removed and no `tables_select` check, so the difference between "Workspace version not found" and a successful lookup was a uid-existence oracle for tables the caller may not read. `workspace_changes_list` likewise listed uid, pid, state and stage for every workspace-aware table regardless of the grant. The lookup now requires the table to be workspace-aware *and* readable by the caller, and the listing is filtered the same way. The publish/discard/stage authorization itself was never affected — that has always been DataHandler's and the workspaces hook's decision.
- **A workspace version can only be acted on from its own workspace.** The lookup is now bound to `t3ver_wsid = <the caller's workspace>`, so a member of one workspace can no longer publish or discard another's work by uid. Every refusal — unknown uid, another workspace, an unreadable or non-versioned table — returns the same message, which names `workspace_switch` so a client can correct itself.
- **The backend module validates redirect URIs.** `ClientRepository::validateRedirectUrisForRegistration()` — HTTPS, an `http` loopback address or a reverse-domain private-use scheme, no fragment, length and count caps — was applied only to RFC 7591 self-registration. A client created or edited in the module stored whatever was typed, so an administrator could save `http://remote-host/cb` or a URI with a fragment and quietly weaken the exact-match guarantee the redirect matcher rests on. Both actions now run the same validator and report its message as a flash error instead of saving. One policy rather than two.
- **Columns marked `exclude` in TCA now require the `non_exclude_fields` grant.** The read surface returned every column of a value or relation type to anyone holding `tables_select` on the table, so a non-admin could read `pages.TSconfig`, `tt_content.pi_flexform`, the `starttime` / `endtime` / `fe_group` fields and usually `hidden` — columns TYPO3 withholds from them. `exclude` is not an edit-only flag: core's `DatabaseRecordList` builds its columns from `BackendUtility::getAllowedFieldsForTable()`, which applies exactly this check, so such a user is not shown the column in the list module either. `TcaSchemaService` now consults the grant in `getReadFields()`, `getListFields()`, `getWritableFields()`, `getFieldsSchema()` and `getMMFields()`, which covers every list, get, search and schema path as well as the generated tool descriptions. Administrators hold every grant, so their view is unchanged; with no authenticated backend user the fields are withheld rather than exposed. Writes were already safe — DataHandler applies the same check — but naming a withheld field now lands it in `ignoredFields` instead of being dropped inside DataHandler.
...
- **The OAuth client's "Backend User" restriction is now enforced.** The backend module has always offered "Backend User (optional, restricts authorization to this user)" on a client and stored the choice in `tx_msmcpserver_oauth_client.be_user`, but nothing read it back: the consent screen, the code exchange and the refresh all took whichever backend user held the session. An administrator who bound a client to one account was relying on a control that did not exist. `ClientRepository::restrictsToAnotherUser()` is now asked at every point a client turns into a grant — the authorize GET renders an access-denied page instead of the consent form (naming the signed-in account, with an RFC 6749 `error=access_denied` link back to the application), the authorize POST refuses with `403 access_denied`, `exchangeCode()` refuses a code minted for a different account or for a client deleted since, and `refreshToken()` refuses and revokes the whole token family when the client has since been bound to someone else, so re-assigning a client cuts off the account it used to serve. Clients left at "Any user" (`be_user = 0`) behave as before.
- **The OAuth consent screen now applies TYPO3's mandatory-MFA setup gate.** The `requireMfa` policy (`BE/requireMfa`, or user TSconfig `auth.mfa.required`) is enforced by core only in the backend middleware stack: a user who has passed the password but never set a provider up is redirected to the setup screen on every backend route. `/mcp/oauth/authorize` runs in the frontend stack, bootstrapped the backend user with a bare `start()` and treated any uid as authenticated — so such a user (or whoever held their password) could open the authorize URL directly, click Authorize and hold a full-rights token for months while the backend itself refused them every module. `resolveAuthenticatedBackendUserUid()` now mirrors core's check (`isMfaSetupRequired()`, honouring an already-passed `mfa` session flag and switch-user mode) and treats the user as not authenticated: the GET redirects to the backend login, which routes them into MFA setup and, via the continuation cookie, back to the consent screen once it is done; the POST answers `401 login_required`; the post-login bounce does not fire.
- **The consent screen now says where the authorization goes and whether anyone vetted the client.** RFC 7591 registration is unauthenticated by design, so anyone could register a client named `Claude Desktop` with an HTTPS redirect URI on their own host, send a backend user the authorize link, and receive a full-rights code from a consent screen that showed nothing but that name. The screen now names the redirect target (`client.example:8443`, or the whole URI for a private-use scheme) and, for a client that registered itself rather than being created in the backend module, carries a warning that no administrator has verified it. Client names are normalised at registration — control and format characters stripped (a bidi override reverses the displayed name, zero-width joiners hide in it), whitespace collapsed, capped at 255 — instead of merely truncated.
- **Dynamic client registration can be switched off.** New setting `dynamicClientRegistrationEnabled` (default `1`, so nothing changes on upgrade). With `0`, `/mcp/oauth/register` answers `403 access_denied` and the RFC 8414 metadata omits `registration_endpoint`, so an installation that provisions every client in the backend module no longer exposes the endpoint at all. `OAuthMiddleware` takes a new `DynamicRegistrationPolicy` constructor argument; flush the TYPO3 caches after deploying.

### Fixed
- **A backend session with its MFA challenge still open no longer crashes the authorize endpoint.** `BackendUserAuthentication::start()` throws `MfaRequiredException` for that state; the middleware did not catch it, so the request died with an uncaught exception (a 500 — fail-closed, but no way forward). The exception is now caught, the half-authenticated object is not left in `$GLOBALS['BE_USER']`, and the user is sent through the backend login, which takes them to the MFA form.
...
- **`pages_update` with a new slug over HTTP reported an internal error after the write had succeeded.** TYPO3's redirects extension reacts to a slug change from `processDatamap_afterDatabaseOperations` — after the row is written — and ends in `BackendUtility::setUpdateSignal()`, which hashes the backend user's session id. Core skips that on CLI only; the backend user the extension bootstraps for the HTTP endpoint had no session, so the hook died with "Call to a member function getIdentifier() on null". Title, slug and hidden were stored, the client was told the tool had failed. `BackendUserBootstrap` now gives the user an anonymous, never-persisted session (`initializeSession()`). The stdio transport was not affected.
- **Write failures are no longer a generic "An internal error occurred".** `DataHandlerService` relays DataHandler's `errorLog` — TYPO3's own editor-facing refusals such as "Attempt to modify record … without permission" — as the tool error. When a DataHandler hook throws, the tool reports the exception class and code, what was being done (`updating pages:68`) and that the change **may already have been applied**, so a client knows to read the record back; the raw message goes to the TYPO3 log only, since it can embed SQL or file paths.
- **`record_search` / `record_count` rejected a plain-text `search` on tables other than pages and content.** `search: "Brio"` on a dynamic table answered "search must be a JSON object, but is not valid JSON: Syntax error", although `pages_search` and `content_search` accept a bare term. Both tools now LIKE-match a plain-text term against the table's label field (TCA `ctrl.label`, e.g. `title`); a table that declares none rejects the term with the JSON shape to use instead (`{"code":{"like":"Brio"}}`), and both tool descriptions say so.

### Changed
- README: the `search` parameter's accepted shapes are documented in one table for `record_search` and `record_count`, tool errors from DataHandler are described under Tools Reference, and the AI Client Configuration section notes that clients cache the tool list and must be restarted after an upgrade or after enabling a table.

## [1.3.0] - 2026-09-07

**Upgrading from 1.2.0.** No database changes and no new settings. Two things are visible to a client: `select` / `group` fields with an `MM` table now appear in `table_schema`, `*_get`, `*_list` (via `selectFields`) and `record_search` as lists of related UIDs, and the write tools accept them instead of reporting them in `ignoredFields`. A `record_search` / `record_count` that filtered or ordered on such a field was matching against the relation count before; it now returns an error.
