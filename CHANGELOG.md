...
- **Password resets via TYPO3's PasswordReset service revoke MCP tokens.** The 0.12.4 DataHandler hook did not fire for the "forgot password" email flow or `backend:resetpassword`, which write to `be_users` directly; a listener for `PasswordHasBeenResetEvent` now covers them.
